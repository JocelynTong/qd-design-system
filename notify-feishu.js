/**
 * 千岛设计系统 · 飞书候选池周报
 * 用法: node notify-feishu.js
 * 读取 _candidates.md → 格式化飞书卡片 → POST webhook
 *
...
const candidatesPath = path.join(__dirname, '_candidates.md');
...
function formatRows(rows) {
  ...
  return rows.map(r => `• **${r[0]}** × ${r[2]}次｜最近：${r[3]}${r[4] ? '｜去向：' + r[4] : ''}`).join('\n');
}

const repoUrl = 'https://github.com/jocelyntong/echo-design-system/blob/main/_candidates.md';
