...
**一次性阅读（建议入职时做）：** 运行 `npm run dev`，访问 `http://localhost:8081`，重点看左侧「Token 三级映射原理」面板。不需要记住任何色值，只需要建立一个认知：颜色是有结构的，改底层一个值，所有引用它的地方会自动联动——这就是为什么设计系统能保持一致，也是为什么不能随口说「这里改成紫色」。
